...
public class SDItemModels extends ItemModelProvider {

    public SDItemModels(DataGenerator generator, ExistingFileHelper existingFileHelper) {
    ...
    public void generatedModel(Block item) {
        singleTexture(item.getRegistryName().getPath(), new ResourceLocation("item/generated"), "layer0", modLoc("item/" + item.getRegistryName().getPath()));
    }

    public void trapdoorModel(TrapDoorBlock item) {
